...
# and to give a specific color to each tissue
...
all_counts_merged_tissue = master_table.groupby(['tissue','RT']).count()['gene_id']\
    .reset_index().pivot(columns = 'RT', index = 'tissue', values = 'gene_id').reset_index()
...
# format axis
plt.ylabel('normalized counts (%)', fontsize = 8); plt.xlabel('')
